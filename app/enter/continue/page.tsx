...
export default async function ContinuePage() {
  ...
  return (
    <>
      <a className="skip-link" href="#main">Skip to sealing</a>
      <Masthead formationStatus={"SIGNED IN · " + session.user.email.replace(/^(.).*(@.*)$/, "$1***$2")} />
      <main id="main">
        <section className="entry-instrument ink-section" aria-labelledby="seal-title">
          <div className="entry-copy">
            ...
            <div className="receipt-block">
              <div className="receipt-line"><dt>YOU ACCEPT</dt><dd>Declaration {versions.declaration}</dd></div>
              <div className="receipt-line"><dt></dt><dd>Constitution {versions.constitution}</dd></div>
              <div className="receipt-line"><dt></dt><dd>Relay protocol {versions.protocol}</dd></div>
              <div className="receipt-line"><dt></dt><dd>Privacy notice draft {versions.privacyNotice}</dd></div>
              <div className="receipt-line"><dt></dt><dd>Legal status {versions.legalStatus}</dd></div>
              <div className="receipt-line"><dt>READ THE DOCUMENTS</dt><dd><Link href="/source/FOUNDING-RELAY-PROTOCOL.md" style={{ color: "var(--paper)" }}>RELAY PROTOCOL</Link> · <Link href="/source/CONSTITUTION-0.1.md" style={{ color: "var(--paper)" }}>CONSTITUTION</Link></dd></div>
            </div>
          </div>
