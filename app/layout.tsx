import type { Metadata } from "next";
import "./globals.css";
import { LOCK_LINE } from "@/legal/documents";

export const metadata: Metadata = {
  // Every page that does not set its own title inherits this one. It said
  // "Day 1" until the Founding Million shipped, so a shared link to /enter
  // announced a different project than the one it opened.
  // Default only, no template: every page that sets a title already carries
  // its own suffix, and a template would double it.
  title: "OURS \u00b7 The Founding Million",
  description:
    "The first 1,000,000 people form OURS. One person, one non-transferable Founding Right, free. Legal membership is not yet issued.",
  metadataBase: new URL(process.env.NEXT_PUBLIC_APP_URL ?? "http://127.0.0.1:3000"),
  openGraph: {
    type: "website",
    title: "NOBODY LEAVES FIRST. EVERYBODY LEAVES TOGETHER.",
    description: LOCK_LINE,
  },
  // Without an explicit twitter:card, X falls back to the small imageless
  // summary card even when og:image is present. Observed on the first live
  // share of the homepage, 26 AUG 2026.
  twitter: {
    card: "summary_large_image",
    title: "NOBODY LEAVES FIRST. EVERYBODY LEAVES TOGETHER.",
    description: LOCK_LINE,
  },
  icons: {
    icon: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' fill='%23111111'/%3E%3Ctext x='16' y='22' font-family='monospace' font-size='17' fill='%23f3f0e8' text-anchor='middle'%3EO%3C/text%3E%3C/svg%3E",
  },
};

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <body>{children}</body>
    </html>
  );
}
